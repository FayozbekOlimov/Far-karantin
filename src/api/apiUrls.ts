/* contact */
export const contactDataUrl = 'contact-data'; // c
export const contactForm = 'contact-form'; //C

/* usefullink and social */
export const socialNetworkUrl = 'social-network'; // c
export const usefulLinkUrl = 'useful-link'; // c

/* sayt bo'limlari */
export const bannerImageUrl = 'banner-image';
export const symbolsUrl = 'symbols';
export const menuUrl = 'menu';
export const galleryUrl = 'gallery';
export const videoUrl = 'video';
export const managementUrl = 'management';

export const pageUrl = 'info';
export const leaderUrl = 'leader';
export const newsUrl = 'news';
export const filesUrl = 'files';

export const newsDetailUrl = 'news-detail';
export const bannerNewsUrl = 'banner-news';
export const latestGalleryUrl = 'latest-gallery';
export const latestVideoUrl = 'video/latest';
export const cardLinksUrl = 'card-link';
export const adsImgUrl = 'ad-pictures';